...
# Read the dataset
df <- read.csv("../../../datasets/acleddata/entity-extraction-antisemitism-ACLED/experiment-1/sample.csv", 
               sep=",", 
               header=TRUE, 
               quote = "\"") %>% 
  na.omit()
...
if ("notes" %in% colnames(df) && !all(is.na(df$notes))) {
  ...
  df <- df %>%
    rowwise() %>%
    mutate(auto = list(extract_organizations(notes))) %>%
    unnest(auto) %>%
    select(event_id_cnty, auto)

  
  # Write the results to a CSV file
  write.csv(df, "../../../datasets/acleddata/entity-extraction-antisemitism-ACLED/experiment-1/auto.csv", row.names=FALSE)
}
